type write_mode = Append | Overwrite

type bfs_traversal_direction = Top_to_bottom | Bottom_to_top

type root_type = Top | Bottom
...
module type S = sig
  ...
  type snapshot_diff = { removed_nodes : id list;
                         changed_nodes : id list;
                         new_nodes     : id list;
                       }

  type node_record = { id            : id;
                       node          : node option;
  ...
  type 'a traversal_function = Full_traversal         of ('a -> id -> node -> t -> 'a * t)
  ...
  val add_node     : write_mode -> id -> node -> ?parents:id list -> ?children:id list -> ?group:id -> t -> t
  ...
  val linear_traverse     : ?ids:id list -> 'a -> 'a traversal_function -> t -> 'a * t
  val linear_traverse_rev : ?ids:id list -> 'a -> 'a traversal_function -> t -> 'a * t
  val bfs_traverse        : id -> 'a -> bfs_traversal_direction -> 'a traversal_function -> t -> 'a * t
  val bfs_traverse_rev    : id -> 'a -> bfs_traversal_direction -> 'a traversal_function -> t -> 'a * t
  ...
  val compress_bfs   : ?compress_leaf:bool -> bfs_traversal_direction -> id -> t -> t
  ...
  val refresh_node_graphics : height:int -> ?redo_layout:bool -> ?label_affect_dag:bool -> Dagre.t Js_of_ocaml.Js.t -> Cytoscape.t Js_of_ocaml.Js.t -> t -> t -> t

  val plot : height:int -> ?show_label:bool -> ?label_affect_dag:bool -> Dagre.t Js_of_ocaml.Js.t option -> Cytoscape.t Js_of_ocaml.Js.t -> t -> t
end

module Make (B : Base) : S
  with type id   = B.id
  with type data = B.data
  with type node = B.node
= struct
  include B
  ...
  type t = {
    ...
    group_lookup         : id IdMap.t;      (* lookup group that an id belongs to *)
    group_members_lookup : id list IdMap.t; (* lookup members that a group contains *)
  ...
  module Helpers : sig
    ...
    val add_single_to_bucket       : bucket_id:id -> data:'a -> 'a list IdMap.t -> 'a list IdMap.t
    val add_list_to_bucket         : bucket_id:id -> data:'a list -> 'a list IdMap.t -> 'a list IdMap.t
    val add_single_to_buckets      : bucket_ids:id list -> data:'a -> 'a list IdMap.t -> 'a list IdMap.t
    val add_list_to_buckets        : bucket_ids:id list -> data:'a list -> 'a list IdMap.t -> 'a list IdMap.t
    val remove_single_from_bucket  : data:'a -> bucket_id:id -> 'a list IdMap.t -> 'a list IdMap.t
    val remove_list_from_bucket    : data:'a list -> bucket_id:id -> 'a list IdMap.t -> 'a list IdMap.t
    val remove_single_from_buckets : data:'a -> ?bucket_ids:id list -> 'a list IdMap.t -> 'a list IdMap.t
    val remove_list_from_buckets   : data:'a list -> ?bucket_ids:id list -> 'a list IdMap.t -> 'a list IdMap.t
    ...
    let map_with_ids ~(ids : id list) (f : 'a -> 'a) (m : 'a IdMap.t) : 'a IdMap.t =
      let rec aux ids f m =
        match ids with
        | []      -> m
        | x :: xs ->
    ...
    let add_single_to_bucket ~(bucket_id : id) ~(data : 'a) (m : 'a list IdMap.t) : 'a list IdMap.t =
    ...
    let add_list_to_bucket ~(bucket_id : id) ~(data : 'a list) (m : 'a list IdMap.t) : 'a list IdMap.t =
      match IdMap.find_opt bucket_id m with
      | None   -> IdMap.add bucket_id (List.sort_uniq compare data)       m
      | Some l -> IdMap.add bucket_id (List.sort_uniq compare (data @ l)) m

    let add_single_to_buckets ~(bucket_ids : id list) ~(data : 'a) (m : 'a list IdMap.t) : 'a list IdMap.t =
    ...
    let add_list_to_buckets ~(bucket_ids : id list) ~(data : 'a list) (m : 'a list IdMap.t) : 'a list IdMap.t =
      let rec aux ids data m =
        match ids with
        | []      -> m
    ...
    let remove_single_from_bucket ~(data : 'a) ~(bucket_id : id) (m : 'a list IdMap.t) : 'a list IdMap.t =
      let filter l =
        List.filter (fun x -> x <> data) l
      in
      match IdMap.find_opt bucket_id m with
      | None   -> m
      | Some d -> IdMap.add bucket_id (filter d) m

    let remove_list_from_bucket ~(data : 'a list) ~(bucket_id : id) (m : 'a list IdMap.t) : 'a list IdMap.t =
      let filter l =
        List.filter (fun x -> not (List.mem x data)) l
      in
      match IdMap.find_opt bucket_id m with
      | None   -> m
      | Some d -> IdMap.add bucket_id (filter d) m

    let remove_single_from_buckets ~(data : 'a) ?(bucket_ids : id list option) (m : 'a list IdMap.t) : 'a list IdMap.t =
      let filter l =
        List.filter (fun x -> x <> data) l
      in
    ...
    let remove_list_from_buckets ~(data : 'a list) ?(bucket_ids : id list option) (m : 'a list IdMap.t) : 'a list IdMap.t =
      let filter l =
        List.filter (fun x -> not (List.mem x data)) l
      in
      match bucket_ids with
    ...
    let is_visible (id : id) (m : t) : bool =
      IdMap.find id m.node_visibility
    ...
    let raise_exn_if_missing ?(msg : string = "Entry is missing in map") (id : id) (m : 'a IdMap.t) : 'a IdMap.t =
      match IdMap.find_opt id m with
      | None   -> failwith msg
      | Some _ -> m
    ...
    let look_up_respect_vis (id : id) (m : 'a IdMap.t) (main : t) : 'a =
      ...
      if visible then IdMap.find id m
      else            failwith "Node is not visible"

    let look_up_opt_respect_vis (id : id) (m : 'a IdMap.t) (main : t) : 'a option =
      let visible = find_with_default id true main.node_visibility in
      if visible then IdMap.find_opt id m
      else            None
    ...
    let filter_invisible_ids (ids : id list) (m : t) : id list =
      let rec aux ids acc =
        match ids with
        | []      -> List.rev acc
        | x :: xs -> aux xs (if IdMap.find x m.node_visibility then x :: acc else acc)
      in
      aux ids []
  end
  ...
  let label (id : id) (node : node) (m : t) : string =
    ...
    if visible then
      label_internal id node
    else
      ""

  let color (id : id) (node : node) (_m : t) : string =
    color_internal id node

  let size (id : id) (node : node) (_m : t) : int =
    size_internal id node
  ...
  let append_parents_children_linkage
      ~(id       : id)
      ~(parents  : id list option)
      ~(children : id list option)
      (m : t)
  ...
  let remove_parents_children_linkage
      ?(remove_parents_linkage  : bool = true)
      ?(remove_children_linkage : bool = true)
      ~(id : id)
      (m   : t)
    : t =
    let open Helpers in
    ...
    let children        = match IdMap.find_opt id children_lookup with
      | None   -> []
      | Some l -> l
    in
    let parents         = match IdMap.find_opt id parents_lookup  with
      | None   -> []
      | Some l -> l
    in
    ...
    let (children_lookup, parents_lookup) =
      if remove_parents_linkage then
        let c = children_lookup
                |> remove_single_from_buckets ~data:id ~bucket_ids:parents
        in
        let p = parents_lookup
                |> IdMap.remove id
        in
        (c, p)
      else
        (children_lookup, parents_lookup)
    in
    let (children_lookup, parents_lookup) =
  ...
  let remove_node (id : id) (m : t) : t =
    ...
    let { node_store;
          group_lookup;
          group_members_lookup; _ } = m in

    let group                = IdMap.find_opt id group_lookup in

    let node_store           = IdMap.remove id node_store in
    let group_lookup         = group_lookup
                               |> IdMap.remove id
    in
    let group_members_lookup = match group with
      | None       -> group_members_lookup
      | Some group -> group_members_lookup
  ...
  let remove_nodes (ids : id list) (m : t) : t =
    let rec aux ids m =
      match ids with
      | [] -> m
      | x :: xs -> aux xs (remove_node x m)
    in
    aux ids m

  let add_node_via_record (write_mode : write_mode) (record : node_record) (m : t) : t =
    let open Helpers in
    ...
    let node_store           = match node with
      | None      -> raise_exn_if_missing id node_store ~msg:"Node is missing in node store"
      | Some node -> IdMap.add id node node_store
    in
    let group_lookup         = match group with
      | None       -> group_lookup
      | Some group -> IdMap.add id group group_lookup
    in
    let node_visibility = match node_visible with
      | None   -> add_to_map_if_missing id true node_visibility
      | Some v -> IdMap.add id v node_visibility
    in
    let label_visibility = match label_visible with
      | None   -> add_to_map_if_missing id false label_visibility
      | Some v -> IdMap.add id v label_visibility
    in
    let group_members_lookup = match group with
      | None       -> group_members_lookup
      | Some group -> add_single_to_bucket ~bucket_id:group ~data:id group_members_lookup
    in
    ...
    | Overwrite ->
      let remove_parents_linkage  = match parents with
        | None   -> false
        | Some _ -> true
      in
      let remove_children_linkage = match children with
        | None   -> false
        | Some _ -> true
      in
      ...
      |> remove_parents_children_linkage
        ~remove_parents_linkage
        ~remove_children_linkage
        ~id
  ...
  let add_nodes_via_records (write_mode : write_mode) (records : node_record list) (m : t) : t =
  ...
  let edges_of_node (id : id) (m : t) : (id * id) list =
    ...
    let parents_edges = List.fold_left
        (fun acc x ->
           (x, id) :: acc
        )
        []
        parents
    in
    let children_edges = List.fold_left
        (fun acc x ->
           (id, x) :: acc
        )
        []
  ...
  let linear_traverse_internal
      ~(respect_visibility : bool)
      ?(ids                : id list option)
      (ctx                 : 'a)
      (f                   : 'a traversal_function)
      (m                   : t)
    : 'a * t =
    let rec aux look_up_opt ids ctx f m =
      ...
      | x :: xs ->
        ...
        | Some n ->
          let (terminate, new_ctx, new_m) = match f with
            | Full_traversal         f -> let (new_ctx, new_m) = f ctx x n m in (false, new_ctx, new_m)
            | Partial_traversal      f -> f ctx x n m
            | Full_traversal_pure    f -> (false, f ctx x n m, m)
            | Partial_traversal_pure f -> let (term, new_ctx) = f ctx x n m in (term, new_ctx, m)
          in
          if terminate then
            (new_ctx, new_m)
          else
            aux look_up_opt xs new_ctx f new_m
    in
    let look_up_opt =
      if respect_visibility then
        Helpers.look_up_opt_respect_vis
      else
        (fun id m _main -> IdMap.find_opt id m)
    in
    let ids = match ids with
      | None     -> Helpers.id_list m.node_store
      | Some ids -> ids
    in
  ...
  let linear_traverse_ignore_vis
      ?(ids : id list option)
      (ctx  : 'a)
      (f    : 'a traversal_function)
      (m    : t)
  ...
  let linear_traverse_rev_ignore_vis
      ?(ids : id list option)
      (ctx  : 'a)
      (f    : 'a traversal_function)
      (m    : t)
  ...
  let bfs_traverse_internal
      ~(respect_visibility : bool)
      (id                  : id)
      (ctx                 : 'a)
      (direction           : bfs_traversal_direction)
      (f                   : 'a traversal_function)
      (m                   : t)
    : 'a * t =
    ...
    while Queue.length queue > 0 do
      ...
      | Some cur ->
        (* executes operation *)
        let (terminate, new_ctx, new_m) = match f with
          | Full_traversal         f -> let (new_ctx, new_m) = f !ctx id cur !m in (false, new_ctx, new_m)
          | Partial_traversal      f -> f !ctx id cur !m
          | Full_traversal_pure    f -> (false, f !ctx id cur !m, !m)
          | Partial_traversal_pure f -> let (term, new_ctx) = f !ctx id cur !m in (term, new_ctx, !m)
        in
        m   := new_m;
        ctx := new_ctx;
        if terminate then
  ...
  let bfs_traverse_rev
      ...
      (m         : t)
    : 'a * t =
    let record_ids acc id _node _m =
      id :: acc
    in
    let (ids, m) = bfs_traverse id [] direction (Full_traversal_pure record_ids) m in
    let (ctx, m) = linear_traverse ~ids ctx f m in
    (ctx, m)

  let bfs_traverse_rev_ignore_vis
      (id        : id)
      (ctx       : 'a)
      (direction : bfs_traversal_direction)
      (f         : 'a traversal_function)
    ...
    let record_ids acc id _node _m =
      id :: acc
    in
    let (ids, m) = bfs_traverse_ignore_vis id [] direction (Full_traversal_pure record_ids) m in
  ...
  let update_list_node_visibility (ids : id list) (visibility : bool) (m : t) : t =
  ...
  let restore_node_visibility ?(ids : id list option) ~(old_m : t) (m : t) : t =
    let rec aux ids old_m m =
      ...
      | []      -> m
      | x :: xs ->
        let visibility = Helpers.find_with_default x false old_m.node_visibility in
  ...
  let filter (pred : id -> node -> t -> bool) (m : t) : t =
    ...
    let pred =
      (fun id node ->
         if Helpers.is_visible id m then
           pred id node m
         else
           true
      )
  ...
  let filter_first_opt (pred : id -> node -> t -> bool) (m : t) : id option =
    let (res, _) = linear_traverse
        None
        (Partial_traversal_pure
           (fun _acc id node m ->
              if pred id node m then
                (true, Some id)
              else
                (false, None)
           ))
  ...
  let diff (root_type : root_type) (m1 : t) (m2 : t) : t * t =
    let same (id1 : id) (n1 : node) (m1 : t) (id2 : id) (n2 : node) (m2 : t) : bool =
    ...
    in
    let rec remove_common (id1 : id) (n1 : node) (m1 : t) (id2 : id) (n2 : node) (m2 : t) : t * t =
      if same id1 n1 m1 id2 n2 m2 then
        ...
        (m1, m2)
    in
    let id1 = match find_root_opt root_type m1 with
      | None   -> failwith "No root for n1"
      | Some x -> x
    in
    let id2 = match find_root_opt root_type m2 with
      | None   -> failwith "No root for n2"
      | Some x -> x
    in
    let res = remove_common id1 (find_node id1 m1) m1 id2 (find_node id2 m2) m2 in
    res

  let snapshot_diff
      ?(group_invisible_nodes_as_removed : bool = true)
      ~(check_content                    : bool)
      ~(check_label_visibility           : bool)
      ~(check_node_visibility            : bool)
      ~(check_group                      : bool)
      ~(check_group_members              : bool)
      ~(check_children                   : bool)
      ~(check_parents                    : bool)
      ~(prev                             : t)
      (cur                               : t)
    ...
    let (changed_ids, _) = linear_traverse_ignore_vis ~ids:common_ids
      []
      (Full_traversal_pure
         (fun acc id prev_node _ ->
            let different =
              (check_content &&
               prev_node <> IdMap.find id cur.node_store)
    ...
    let (removed_ids, changed_ids) =
      if group_invisible_nodes_as_removed then
        linear_traverse_ignore_vis ~ids:changed_ids
          (removed_ids, [])
          (Full_traversal_pure
             (fun (removed_ids, changed_ids) id _ cur ->
                let visible = IdMap.find id cur.node_visibility in
                if visible then
                  (removed_ids,       id :: changed_ids)
                else
                  (id :: removed_ids, changed_ids)
             )
          )
          cur
        |> Misc_utils.unwrap_tuple_0
      else
        (removed_ids, changed_ids)
    in
  ...
  let compress_internal (group_type : group_type) (ids : id list) (m : t) : t =
    ...
    | _   ->
          ...
          (Full_traversal
             (fun () id _node m ->
                ...
                let group_members_lookup =
                  Helpers.add_single_to_bucket ~bucket_id:group_id ~data:id group_members_lookup in
      ...
      let ((parents, children), _) = linear_traverse ~ids
          ([], [])
          (Full_traversal_pure
  ...
  let compress_list (ids : id list) (m : t) : t =
    compress_internal List ids m

  let compress_bfs ?(compress_leaf : bool = false) (direction : bfs_traversal_direction) (id : id) (m : t) : t =
    (* quit if the node itself is a leaf *)
    let is_leaf = match direction with
    ...
    if is_leaf then
      ...
      let (ids, m) =
        bfs_traverse id [] direction
          (Full_traversal (fun acc id _node m ->
               let is_group = IdMap.mem id m.group_members_lookup in
               let acc =
                 if is_group then
                   id :: acc
                 else
                   let leaves = match direction with
                     | Bottom_to_top -> find_parents  id m
                     | Top_to_bottom -> find_children id m
                   in
                   match leaves with
  ...
  let recompress (id : id) (direction : bfs_traversal_direction) (m : t) : t =
    ...
    | Some group_id ->
      ...
      | BFS ->
        ...
        let (ids, m) = linear_traverse ~ids:group_members
            []
            (Full_traversal_pure
               (fun acc id _node m ->
                  let (ids, _) = bfs_traverse id
                      []
                      direction
                      (Full_traversal_pure
                         (fun acc id _node m ->
  ...
  let decompress (id : id) (direction : bfs_traversal_direction) (m : t) : t =
    ...
    | Some members ->
      ...
      | Some old_m ->
        ...
        (* collect ids of all nodes in member trees *)
        let (ids, m) = linear_traverse_ignore_vis ~ids:group_members
            []
            (Full_traversal_pure
               (fun acc id _node m ->
                  let (ids, _) = bfs_traverse_ignore_vis id
                      []
                      direction
                      (Full_traversal_pure
                         (fun acc id _node m ->
  ...
  let to_assoc_list (m : t) : (id * node) list =
    IdMap.bindings m.node_store
  ...
  let refresh_node_graphics
      ~(height           : int)
      ?(redo_layout      : bool = true)
      ?(label_affect_dag : bool = true)
      (dagre             : Dagre.t Js_of_ocaml.Js.t)
      (cy                : Cytoscape.t Js_of_ocaml.Js.t)
      (prev              : t)
      (cur               : t)
    : t =
    let remove_node () (id : id) (_node : node) (_cur : t) =
      ...
      if label_affect_dag then
        Dagre.remove_node dagre ~id:id_str;
    ...
    let { removed_nodes;
          changed_nodes;
          new_nodes; } = snapshot_diff
        ~check_content:false
        ~check_label_visibility:true
        ~check_node_visibility:true
        ~check_group:false
        ~check_group_members:false
        ~check_children:true
        ~check_parents:true
        ~prev
        cur
    ...
    if redo_layout then
      ...
      |> linear_traverse_ignore_vis ~ids:removed_nodes () (Full_traversal_pure remove_node)       |> Misc_utils.unwrap_tuple_1
      |> linear_traverse_ignore_vis ~ids:changed_nodes () (Full_traversal_pure remove_node)       |> Misc_utils.unwrap_tuple_1

      |> (fun m ->
          if label_affect_dag then
            m
            (* add changed, new nodes to Dagre *)
            |> linear_traverse            ~ids:changed_nodes () (Full_traversal_pure add_node_to_dagre) |> Misc_utils.unwrap_tuple_1
            |> linear_traverse            ~ids:new_nodes     () (Full_traversal_pure add_node_to_dagre) |> Misc_utils.unwrap_tuple_1

            (* add edges of changed, new nodes to dagre *)
            |> linear_traverse            ~ids:changed_nodes () (Full_traversal_pure add_node_edges_to_dagre) |> Misc_utils.unwrap_tuple_1
            |> linear_traverse            ~ids:new_nodes     () (Full_traversal_pure add_node_edges_to_dagre) |> Misc_utils.unwrap_tuple_1
      ...
      |> linear_traverse            ~ids:all_cur_ids () (Full_traversal_pure sync_dagre_xy_to_table) |> Misc_utils.unwrap_tuple_1

      (* add changed, new nodes to Cy *)
      |> linear_traverse            ~ids:changed_nodes () (Full_traversal_pure add_node_to_cy) |> Misc_utils.unwrap_tuple_1
      |> linear_traverse            ~ids:new_nodes     () (Full_traversal_pure add_node_to_cy) |> Misc_utils.unwrap_tuple_1

      (* set node attributes in cy*)
      |> linear_traverse            ~ids:changed_nodes () (Full_traversal_pure set_node_attributes_cy) |> Misc_utils.unwrap_tuple_1
      |> linear_traverse            ~ids:new_nodes     () (Full_traversal_pure set_node_attributes_cy) |> Misc_utils.unwrap_tuple_1

      (* add edges of changed, new nodes to Cy *)
      |> (fun m ->
          let edges = List.concat (List.map (fun id -> edges_of_node id m) changed_nodes)
                      @ List.concat (List.map (fun id -> edges_of_node id m) new_nodes) in
          let edges = List.sort_uniq compare edges in
          add_edges_to_cy edges;
          m
        )
      ...
      |> linear_traverse            ~ids:all_cur_ids   () (Full_traversal_pure sync_node_xy_to_cy)   |> Misc_utils.unwrap_tuple_1
  ...
  let plot ~(height : int) ?(show_label : bool option) ?(label_affect_dag : bool = true) (dagre : Dagre.t Js_of_ocaml.Js.t option) (cy : Cytoscape.t Js_of_ocaml.Js.t) (m : t) : t =
    let dagre = match dagre with
      | None -> Dagre.make ()
      | Some dagre -> dagre
    in
    ...
    |> (fun m ->
        if label_affect_dag then
          m
          |> linear_traverse () (Full_traversal_pure add_node_to_dagre)     |> Misc_utils.unwrap_tuple_1
          |> linear_traverse () (Full_traversal_pure add_edge_to_dagre)     |> Misc_utils.unwrap_tuple_1
          |> (fun m -> Dagre.layout dagre; m)
          |> linear_traverse () (Full_traversal_pure sync_dagre_xy_to_node) |> Misc_utils.unwrap_tuple_1
        else
          m
      )
    |> linear_traverse () (Full_traversal_pure add_node_to_cy)        |> Misc_utils.unwrap_tuple_1
    |> linear_traverse () (Full_traversal_pure add_edge_to_cy)        |> Misc_utils.unwrap_tuple_1
    |> (fun m -> Cytoscape.end_batch cy; Cytoscape.style_update cy; m)
end
