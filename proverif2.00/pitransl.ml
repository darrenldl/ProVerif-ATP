...
module Transl_process_ctx = struct
  type t = {
    out_pos : Out_pos.out_pos;
    proc_name : string option;
    proc_out_count : int;
  }

  let make out_pos = { out_pos; proc_name = None; proc_out_count = 0 }

  let reset_proc_out_count ctx = { ctx with proc_out_count = 0 }

  let set_proc_name proc_name ctx = { ctx with proc_name = Some proc_name }
end

let rec transl_process cur_state ctx p =
  let open Transl_process_ctx in

  match p with
    Nil -> ()
  | NamedProcess(proc_name, _, p) ->
    begin
      let ctx = ctx
                |> reset_proc_out_count
                |> set_proc_name proc_name
      in
      transl_process cur_state ctx p
    end
  | Par(p,q) -> transl_process cur_state ctx p;
    transl_process cur_state ctx q
  | (Repl (p,occ)) as p' ->
      ...
      transl_term_incl_destructor (no_fail (fun cur_state1 pat1 ->
          end_destructor_group (fun cur_state2 ->
              transl_process { cur_state2 with
                               hyp_tags = (TestTag occ) :: cur_state2.hyp_tags } ctx p
            ) occ { cur_state1 with
          ...
          end_destructor_group (fun cur_state2 ->
              ...
              Terms.auto_cleanup (fun _ ->
                  try
                    ...
                    transl_process { cur_state2 with
                                     constra = constra';
                                     hyp_tags = (TestTag occ) :: cur_state2.hyp_tags } ctx q
                  with Terms.Unify -> ()
                );
            ) occ cur_state1
        )) cur_state (OTest(occ)) t
    ...
    begin
      match tc with
        ...
        transl_pat Always (fun cur_state1 pat1 ->
            end_destructor_group (fun cur_state2 -> transl_process cur_state2 ctx p) occ
              { cur_state1 with
        ...
        transl_term_incl_destructor (no_fail (fun cur_state1 pat1 ->
            end_destructor_group (fun cur_state2 ->
                ...
                transl_pat Always (fun cur_state3 pat2 ->
                    end_destructor_group (fun cur_state4 -> transl_process cur_state4 ctx p) occ
                      { cur_state3 with
  ...
  | Output(tc,t,p,occ) ->
    begin
      ...
      let t = match ctx.out_pos with
        | Spass | Tptp -> (
            let tag = match ctx.proc_name with
              | None -> Printf.sprintf "STEP_%d" ctx.proc_out_count
              | Some name -> Printf.sprintf "%s_STEP_%d" name ctx.proc_out_count
            in
            let term_type = match t with
              | Var b -> b.btype
              | FunApp (f, _) -> let (_, rt) = f.f_type in rt
            in
            let funsymb = { f_orig_name = "";
                            f_name = "";
                            f_type = [term_type], ;
                            f_cat = Types.Tuple;
                            f_initial_cat = Types.Tuple;
                            f_private = false;
                            f_options = 0;
                          }
            in
            FunApp (funsymb, [t; tag])
          )
        | Solve -> t
      in
