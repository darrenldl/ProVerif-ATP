  ...
  let rec expr_to_string f =
    ...
    | UnaryOp (op, f) ->
      Printf.sprintf "%s%s" (unary_op_to_string op)
        ( match f with
          | Variable _ | Function _ | UnaryOp _ -> expr_to_string f
          | _ -> Printf.sprintf "(%s)" (expr_to_string f) )
    | BinaryOp (op, l, r) ->
      Printf.sprintf "%s %s %s"
        ( match l with
          | Variable _ | Function _ | UnaryOp _ -> expr_to_string l
          | _ -> Printf.sprintf "(%s)" (expr_to_string l) )
        (binary_op_to_string op)
        ( match r with
          | Variable _ | Function _ | UnaryOp _ -> expr_to_string r
            ...
            (String.concat ", "
               (List.map
                  (function
                    ...
                    | Pat e -> Printf.sprintf "=%s" (expr_to_string e))
                  args))
            (expr_to_string e) )
    | IfElse x -> (
      ...
      | Process (proc, ps) ->
        ...
        List.iter
          (fun (instance_ty, p) ->
             ...
             | Proc es ->
               ( match instance_ty with
                 | Single -> Print_context.push ctx "| ("
                 | Multi -> Print_context.push ctx "| ! (" );
               Print_context.incre_indent ctx;
               pv_proc_exprs_to_string ctx es;
               Print_context.push ctx ")";
               Print_context.decre_indent ctx )
