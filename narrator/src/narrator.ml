...
let attack_trace node_map =
  ...
  let max_proc_name_len_on_left = List.fold_left
      (fun cur_max s ->
      ...
      steps
  in
  let max_proc_name_len_on_right = List.fold_left
      (fun cur_max s ->
         match s.direction, s.proc_name with
  ...
  String.concat "\n"
    (List.mapi
       (fun global_step_num s ->
          ...
          let proc_name = match s.proc_name with
            | None -> "GLOBAL"
            | Some x -> x
          in
          ...
          | Client_to_intruder -> Printf.sprintf "%d.    %s.%d%s    %s%s -> %s%s : %s\n"
                                    global_step_num
                                    proc_name
                                    s.step_num
                                    proc_name_padding_on_right
                                    proc_name_padding_on_left
                                    proc_name
                                    intruder_name
                                    intruder_name_padding_on_right
                                    expr
          | Intruder_to_client -> Printf.sprintf "%d.    %s.%d%s    %s%s -> %s%s : %s\n"
                                    global_step_num
                                    proc_name
                                    s.step_num
                                    proc_name_padding_on_right
                                    intruder_name_padding_on_left
                                    intruder_name proc_name
                                    proc_name_padding_on_right
                                    expr
       )
       steps)
...
let () =
  ...
  let rec vampire_file_reader () =
    ...
    begin
      ...
      | Ok node_map ->
        ...
        let node_map = Vampire.Analyzed_graph.update_label_visibility false node_map in
        Vampire.Analyzed_graph.plot !dagre_main cy_main ~height:main_dag_node_height node_map |> ignore;
        js_ctx.full_node_map <- Some (Vampire node_map);
        js_ctx.cur_node_map  <- Some (Vampire node_map);
        let attack_trace_box = Js.Unsafe.global##.document##getElementById attack_trace_box_ID in
  ...
  let rec pv_file_reader () =
    ...
    begin
      ...
      | Ok s ->
        ...
        let main_text_display = Js.Unsafe.global##.document##getElementById main_text_display_ID in
  ...
  in
  Cytoscape.on cy_main ~event:"boxstart"
    ~handler:(fun _ev ->
        Lwt_js.yield () >>=
        (fun () ->
  ...
  Cytoscape.on cy_main ~event:"box" ~selector:"node"
    ~handler:(fun ev ->
        Lwt_js.yield () >>=
        (fun () ->
  ...
  Cytoscape.on cy_main ~event:"tap"
    ~handler:(fun ev ->
  ...
  Cytoscape.on cy_main ~event:"tap" ~selector:"node"
    ~handler:(fun ev ->
        ...
        | Some Vampire m ->
          ...
          | Data data ->
            ...
            let single_formula_box = Js.Unsafe.global##.document##getElementById single_formula_box_ID in
            single_formula_box##.innerHTML := Js.string expr_str;

            let em   = Vampire.expr_to_node_map data.expr in
            Vampire.Analyzed_expr_graph.plot None cy_single_nodeAST ~height:expr_AST_height ~show_label:true em |> ignore;

            let single_explanation_box = Js.Unsafe.global##.document##getElementById single_explanation_box_ID in
            single_explanation_box##.innerHTML := Js.string (Vampire.derive_explanation_to_string (Vampire.explain_construction_single target_id m));
          | Group ->
            ()
      );
  Cytoscape.on cy_main ~event:"tap" ~selector:"edge"
    ~handler:(fun ev ->
        Js_utils.set_display ~id:single_formula_box_ID    ~on:false;
        Js_utils.set_display ~id:single_nodeAST_ID        ~on:false;
        ...
        | Some Vampire m ->
          ...
          | (Data source_data, Data target_data) ->
            ...
            let (souce_diff, target_diff) = Vampire.Analyzed_expr_graph.diff Top source_em target_em in
            Cytoscape.reset_style cy_nodeAST1;
            Cytoscape.reset_style cy_nodeAST2;
            Vampire.Analyzed_expr_graph.plot None cy_nodeAST1 ~height:expr_AST_height ~show_label:true source_em |> ignore;
            Vampire.Analyzed_expr_graph.plot None cy_nodeAST2 ~height:expr_AST_height ~show_label:true target_em |> ignore;
      ...
      (fun _ev ->
         begin
           ...
           | Some Vampire old_m ->
               ...
               refresh_node_graphics ~height:main_dag_node_height ~label_affect_dag:false (Misc_utils.unwrap_opt !dagre_main) cy_main old_m new_m |> ignore;
               js_ctx.box_selected_nodes <- [];
         end;
         Js._true
      );
  let decompress_button = Dom_html.getElementById_exn "decompressButton" in
  decompress_button##.onclick := Dom_html.handler
      (fun _ev ->
         begin
           match js_ctx.cur_node_map with
           | None   -> ()
           | Some Vampire old_m ->
             ...
             | Some id ->
               let open Vampire.Analyzed_graph in
               js_ctx.prev_node_map <- Some (Vampire old_m);
               let new_m = Vampire.Analyzed_graph.(decompress id Bottom_to_top old_m) in
               js_ctx.cur_node_map <- Some (Vampire new_m);
               refresh_node_graphics ~height:main_dag_node_height ~label_affect_dag:false (Misc_utils.unwrap_opt !dagre_main) cy_main old_m new_m |> ignore;
         end;
         Js._true
  ...
  recompress_button##.onclick := Dom_html.handler
      (fun _ev ->
         begin
           match js_ctx.cur_node_map with
           | None   -> ()
  ...
  toggle_label_button##.onclick := Dom_html.handler
      (fun _ev ->
         begin
           match js_ctx.cur_node_map with
           | None   -> ()
           | Some Vampire old_m ->
             ...
             | Some id ->
               ...
               js_ctx.cur_node_map <- Some (Vampire new_m);
               refresh_node_graphics ~height:main_dag_node_height ~label_affect_dag:true (Misc_utils.unwrap_opt !dagre_main) cy_main old_m new_m |> ignore;
         end;
         Js._true
      );
  let toggle_label_skip_layout_calc_button = Dom_html.getElementById_exn "toggleLabelSkipLayoutCalcButton" in
  toggle_label_skip_layout_calc_button##.onclick := Dom_html.handler
      (fun _ev ->
         begin
           match js_ctx.cur_node_map with
           | None   -> ()
           | Some Vampire old_m ->
             ...
             | Some id ->
               ...
               let new_m = update_label_visibility ~ids:[id] (not v) old_m in
               js_ctx.cur_node_map <- Some (Vampire new_m);
               refresh_node_graphics ~height:main_dag_node_height ~redo_layout:false (Misc_utils.unwrap_opt !dagre_main) cy_main old_m new_m |> ignore;
         end;
         Js._true
      );
  let explain_chain_button = Dom_html.getElementById_exn "explainChainButton" in
  explain_chain_button##.onclick := Dom_html.handler
      (fun _ev ->
         Js_utils.set_display ~id:single_formula_box_ID    ~on:true;
         Js_utils.set_display ~id:single_nodeAST_ID        ~on:false;
         Js_utils.set_display ~id:nodeAST1_ID              ~on:false;
         ...
         begin
           ...
           | Some Vampire old_m ->
             ...
             | Some id ->
               ...
               let explanations = List.filter
                   (fun (e : derive_explanation) ->
                      e <> Nothing_to_explain &&
                      e <> Dont_know_how
                   )
                   (explain_construction_single_chained id old_m)
               in
               let explanation_str = String.concat "\n" (List.map derive_explanation_to_string explanations) in
               let chain_explanation_box = Js.Unsafe.global##.document##getElementById chain_explanation_box_ID in
               chain_explanation_box##.innerHTML := Js.string explanation_str;
         end;
         Js._true
      );
  let zoom_in_button = Dom_html.getElementById_exn "zoomInButton" in
  zoom_in_button##.onclick := Dom_html.handler
      (fun _ev ->
         begin
  ...
  let pv_raw_attack_trace_button = Dom_html.getElementById_exn "pvRawAttackTraceButton" in
  pv_raw_attack_trace_button##.onclick := Dom_html.handler
      (fun _ev ->
         Js_utils.set_display ~id:single_formula_box_ID    ~on:true;
         Js_utils.set_display ~id:single_nodeAST_ID        ~on:false;
         Js_utils.set_display ~id:nodeAST1_ID              ~on:false;
  ...
  let pv_processed_attack_trace_button = Dom_html.getElementById_exn "pvProcessedAttackTraceButton" in
  pv_processed_attack_trace_button##.onclick := Dom_html.handler
      (fun _ev ->
         Js_utils.set_display ~id:single_formula_box_ID    ~on:true;
         Js_utils.set_display ~id:single_nodeAST_ID        ~on:false;
         Js_utils.set_display ~id:nodeAST1_ID              ~on:false;
  ...
  let knowledge_graph_button = Dom_html.getElementById_exn "knowledgeGraphButton" in
  knowledge_graph_button##.onclick := Dom_html.handler
      (fun _ev ->
         Js_utils.set_display ~id:single_formula_box_ID    ~on:true;
         Js_utils.set_display ~id:single_nodeAST_ID        ~on:true;
         Js_utils.set_display ~id:nodeAST1_ID              ~on:false;
