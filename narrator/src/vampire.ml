...
module Raw_expr = struct
  ...
  let rec expr_to_string (e : expr) : string =
    ...
    | Function (name, params)   -> sprintf "%s(%s)"
                                     name (join_with_comma
                                             (List.map expr_to_string params))
    | UnaryOp  (op, expr)       ->
      sprintf "%s%s"
    ...
    | Quantified (q, idents, e) -> sprintf "%s [%s] : %s"
                                     (quantifier_to_string q)
                                     (join_with_comma idents)
                                     (expr_to_string e)
    | False                     -> "$false"
  ...
  let rec of_tptp_decl (decl : Tptp_ast.decl) =
    ...
    let aux decl =
      ...
      | Annotated_formula (Fof_annotated f) ->
        ...
        let (descr, parent_ids) = match annotations_to_descr_parent_ids annotations with
          | None -> ("", [])
          | Some p -> p
        in
...
module File_parser = struct
  let parse_refutation_proof_string (input : string) : (Raw_line.line option list, string) result =
...
module Analyzed_expr = struct
  ...
  type variable_map_error = Unexpected_unsure_var
  ...
  type pattern_match_direction = Unidirectional | Bidirectional

  module ExprMap = Map.Make(
    struct
      type t = expr
      let compare = compare
    end)
  ...
  let raw_expr_to_analyzed_expr (raw : Raw_expr.expr) : expr =
    let rec aux (ctx : context list) (raw : Raw_expr.expr) : expr =
      match raw with
      | Variable ident           -> (match is_quantified ctx ident with
      ...
      | Quantified(q, idents, e) -> (
          let new_ctx = { quantifier        = q;
                          quantified_idents = idents} in
  ...
  let rec expr_to_string_debug (e : expr) : string =
    ...
    | Function (name, params)   -> sprintf "%s(%s)"
                                     name (join_with_comma
                                             (List.map expr_to_string_debug params))
    | UnaryOp  (op, expr)       -> sprintf "%s(%s)"
                                     (unary_op_to_string op)
                                     (expr_to_string_debug expr)
    | BinaryOp (op, l, r)       -> sprintf "(%s) %s (%s)"
                                     (expr_to_string_debug l)
                                     (binary_op_to_string op)
  ...
  let update_boundedness (e : expr) (changes : (string * boundedness) list) : expr =
    let rec aux (e : expr) : expr =
      match e with
      | Variable (b, v)          -> (match List.assoc_opt v changes with
          | None        -> Variable (b, v)
          | Some(new_b) -> (match (new_b, b) with
  ...
  let get_vars ?(boundedness : boundedness option) (e : expr) : string list =
    let rec aux (e : expr) : string list =
      match e with
      | Variable (b, name)       -> (match boundedness with
  ...
  let pattern_matches ~(pattern : expr) (expr : expr) : bool =
    let rec aux (pattern : expr) (expr : expr) : bool =
      ...
      | (Variable (Free,   _)      as v1), (Variable (Free, _)        as v2) -> if v1 = v2 then true else false
      | (Variable (Universal, _)        ), (_                              ) -> true
      | (Function (f1, es1)             ), (Function (f2, es2)             ) ->
        if f1 = f2 && List.length es1 = List.length es2 then
          aux_list es1 es2
        else
          false
  ...
  let pattern_search ?(s : ExprSet.t = ExprSet.empty) ~(pattern : expr) (expr : expr) : expr list =
    ...
    and aux_list (pattern : expr) (es : expr list) (s : ExprSet.t) : ExprSet.t =
      List.fold_left
        (fun s e -> aux pattern e s)
        s
        es
    in
    ExprSet.elements (aux pattern expr s)

  let var_bindings_in_pattern_match ?(m : expr VarMap.t = VarMap.empty) ~(pattern : expr) (expr : expr) : expr VarMap.t =
    let rec aux (pattern : expr) (expr : expr) (m : expr VarMap.t) : expr VarMap.t =
      match (pattern, expr) with
      | (Variable (Free, _)        ), (_                     ) -> m
      ...
      | (BinaryOp (_, e1a, e1b)    ), (BinaryOp (_, e2a, e2b)) -> aux_list [e1a; e1b] [e2a; e2b] m
      | (Quantified (_, _, e1)     ), (Quantified (_, _, e2) ) -> aux e1 e2 m
      | (False                     ), (False                 ) -> m
      | (InsertedF _               ), (InsertedF _           ) -> m
      | _                                                      -> raise Unmatching_structure
    and aux_list (es1 : expr list) (es2 : expr list) (m : expr VarMap.t) : expr VarMap.t =
      List.fold_left2
        (fun m e1 e2 -> aux e1 e2 m)
        m
        es1
        es2
    in
  ...
  let var_bindings_compatible ~(smaller : expr VarMap.t) ~(larger : expr VarMap.t) : bool =
  ...
  let fill_in_pattern ~(pattern : expr) (var_bindings : expr VarMap.t) : expr =
    let rec aux (pattern : expr) (var_bindings : expr VarMap.t) : expr =
      ...
      | BinaryOp (op, e1, e2)        -> BinaryOp (op, aux e1 var_bindings, aux e2 var_bindings)
      | Quantified (q, ids, e)       -> Quantified (q, ids, aux e var_bindings)
      | False                   as e -> e
      | InsertedF _             as e -> e
    and aux_list (patterns : expr list) (var_bindings : expr VarMap.t) : expr list =
      List.map
        (fun e -> aux e var_bindings)
        patterns
    in
  ...
  let remove_subsumptions (e : expr) : expr =
    let rec aux (e : expr) : expr =
      ...
      | UnaryOp (op, e) -> UnaryOp (op, aux e)
      | BinaryOp (Or, e, UnaryOp (Not, Variable (_, v))) when has_prefix v "spl" -> e
      | BinaryOp (Or, UnaryOp (Not, Variable (_, v)), e) when has_prefix v "spl" -> e
  ...
  let split_on_and (e : expr) : expr list =
    let rec aux (e : expr) : expr list =
      match e with
      | BinaryOp (And, e1, e2) -> aux e1 @ aux e2
      | _                as e -> [e]
    in
    aux e
  ...
  let split_on_or (e : expr) : expr list =
    let rec aux (e : expr) : expr list =
      match e with
      | BinaryOp (Or, e1, e2) -> aux e1 @ aux e2
      | _                as e -> [e]
    in
    aux e
  ...
  let split_on_impl (e : expr) : (expr * expr) option =
    match e with
    | BinaryOp (Imply, e1, e2) -> Some (e1, e2)
    | _                        -> None
  ...
  let rewrite (base : expr) ~(rewrite_rule_from : expr) ~(rewrite_rule_to : expr) ~(expr_from : expr) : expr =
    let rec aux (base : expr) (expr_from : expr) (to_expr : expr) : expr =
      if base = expr_from then
        to_expr
      else
        match base with
        | Variable _          as e -> e
        ...
        | BinaryOp (op, e1, e2)    -> BinaryOp (op, aux e1 expr_from to_expr, aux e2 expr_from to_expr)
        | Quantified (q, ids, e)   -> Quantified (q, ids, aux e expr_from to_expr)
        | False               as e -> e
        | InsertedF _         as e -> e
    and aux_list (bases : expr list) (expr_from : expr) (to_expr : expr) : expr list =
      List.map
        (fun e -> aux e expr_from to_expr)
        bases
    in

    let var_bindings = var_bindings_in_pattern_match ~pattern:rewrite_rule_from expr_from in
  ...
  let similarity (e1 : expr) (e2 : expr) : float =
    let rec aux (e1 : expr) (e2 : expr) : int =
      ...
      | (Variable (Existential, _ )), (_                        ) -> raise Unexpected_existential_var
      | (_                         ), (Variable (Existential, _)) -> raise Unexpected_existential_var
      | (Function (n1, es1)        ), (Function (n2, es2)       ) -> if n1  = n2  then 2 + aux_list es1 es2               else 0
      | (UnaryOp  (op1, e1)        ), (UnaryOp  (op2, e2)       ) -> if op1 = op2 then 2 + aux e1 e2                      else 0
      | (BinaryOp (op1, e1a, e1b)  ), (BinaryOp (op2, e2a, e2b) ) -> if op1 = op2 then 2 + aux_list [e1a; e1b] [e2a; e2b] else 0
  ...
  let pattern_multi_match_map (exprs1 : expr list) (exprs2 : expr list) : expr ExprMap.t option =
    let all_combinations (exprs1 : expr list) (exprs2 : expr list) : (expr * expr) list =
      let rec aux (acc : (expr * expr) list) (exprs1 : expr list) (exprs : expr list) : (expr * expr) list =
        match exprs1 with
        | []      -> acc
        | p :: ps ->
    ...
    let group_by_exprs (combinations : (expr * expr) list) : expr list ExprMap.t =
      let rec aux (m : expr list ExprMap.t) (combinations : (expr * expr) list) : expr list ExprMap.t =
        match combinations with
        | []      -> m
        | (p, e) :: cs ->
          let m =
    ...
    in
    let generate_possible_solutions (m : expr list ExprMap.t) : expr ExprMap.t list =
      let rec aux (keys : expr list) (m : expr list ExprMap.t) (cur : expr ExprMap.t) : expr ExprMap.t list =
        match keys with
        | []      -> [cur]
        | k :: ks ->
    ...
    in
    let filter_by_non_overlapping_exprs_expressions (l : expr ExprMap.t list) : expr ExprMap.t list =
      let no_overlaps (m : expr ExprMap.t) : bool =
        let rec aux
            (keys        : expr list)
            (m           : expr ExprMap.t)
            (exprs1_used : ExprSet.t)
            (exprs2_used : ExprSet.t)
          : bool =
          match keys with
          | []        -> true
          | pat :: ks ->
            let expr               = ExprMap.find pat m in
            let pattern_no_overlap =
              not (ExprSet.mem pat exprs1_used)
            in
            let expr_no_overlap    =
            ...
            if no_overlaps then
              aux ks m (ExprSet.add pat exprs1_used) (ExprSet.add expr exprs2_used)
            else
              false
        in
        let keys = List.map (fun (k, _) -> k) (ExprMap.bindings m) in
      ...
      List.filter
        ...
        l
    in
    let filter_by_compatible_var_bindings (solutions : expr ExprMap.t list) : expr ExprMap.t list =
      let no_overlaps (m : expr ExprMap.t) : bool =
        let rec aux (keys : expr list) (var_bindings : expr VarMap.t) (m : expr ExprMap.t) : bool =
          match keys with
          | []        -> true
          | pat :: ks ->
            let expr     = ExprMap.find pat m in
            let bindings = var_bindings_in_pattern_match ~pattern:pat expr in
            if var_bindings_compatible ~smaller:bindings ~larger:var_bindings then
              let var_bindings =
    ...
    let solution_score (m : expr ExprMap.t) : float =
      let rec aux (keys : expr list) (score_acc : float) (m : expr ExprMap.t) : float =
        match keys with
        | []        -> score_acc
        | pat :: ks ->
          let expr      = ExprMap.find pat m in
      ...
      aux keys 0.0 m
    in
    let sort_solutions_by_score_descending (solutions : expr ExprMap.t list) : expr ExprMap.t list =
      List.sort
        (fun m1 m2 -> Misc_utils.compare_rev (solution_score m1) (solution_score m2))
        solutions
    in
    ...
    in
    Js_utils.console_log (Printf.sprintf "Number of valid combinations : %d" (List.length valid_combinations));
    let possible_solutions =
      valid_combinations
      |> group_by_exprs
      |> generate_possible_solutions
    in
  ...
  let replace_universal_var_name (original : string) (replacement : string) (e : expr) : expr =
    let rec aux (original : string) (replacement : string) (e : expr) : expr =
      match e with
      | Variable (Universal, name)     -> Variable (Universal, if name = original then replacement else name)
      | Variable _                as e -> e
      | Function (name, es)            -> Function (name, aux_list original replacement es)
      | UnaryOp (op, e)                -> UnaryOp (op, aux original replacement e)
      | BinaryOp (op, e1, e2)          -> BinaryOp (op, aux original replacement e1, aux original replacement e2)
      | Quantified (q, ids, e)         -> Quantified (q, ids, aux original replacement e)
      | False                          -> False
      | InsertedF _               as e -> e
    and aux_list (original : string) (replacement : string) (es : expr list) : expr list =
  ...
  let uniquify_universal_var_names_clause_sets (prefix : string) (ess : expr list list) : expr list list =
    let rec aux (gen_id : unit -> string) (acc : expr list list) (ess : expr list list) : expr list list =
      match ess with
      | []        -> List.rev acc
      | es :: ess ->
        let names    = List.sort_uniq compare (List.concat (List.map universal_var_names es)) in
        let name_map = List.map (fun n -> (n, gen_id ())) names in
        let res      =
          List.map
            (fun e -> replace_universal_var_names name_map e)
            es
        in
  ...
  let uniquify_universal_var_names_generic
    ...
    let rec aux
      ...
      | x :: xs ->
        ...
        let names    = List.sort_uniq compare (List.concat (List.map universal_var_names exprs)) in
        let name_map = List.map (fun n -> (n, gen_id ())) names in
        let res      =
          List.fold_left
  ...
  let var_bindings_in_generic
      ?(m : expr VarMap.t = VarMap.empty)
      ~(get_pairs : 'a -> (expr * expr) list)
      (x : 'a)
    : expr VarMap.t =
  ...
  let var_bindings_in_pairs
      ?(m    : expr VarMap.t = VarMap.empty)
      (pairs : (expr * expr) list)
    : expr VarMap.t =
    List.fold_left
      (fun m (k, v) ->
         m
  ...
  let id_to_string = (fun x -> x)

  type node = Data of data
            | Group

  let first_id = "0"

  let next_id = (fun s -> s ^ "0")

  let label_internal (_id : id) (node : node) : string =
    match node with
    | Data node -> node.expr_str
    | Group     -> "Group"

  let color_internal (_id : id) (_node : node) : string =
    "#67cd61"
    (* match node.core with
     * | Some c -> "#67cd61"
     * | None   -> "#800000" *)

  let size_internal (_ : id) (_ : node) : int =
    40

  let node_shape_internal (_ : id) (_ : node) : Cytoscape.node_shape =
    Circle
end

module Analyzed_expr_graph = Graph.Make(Analyzed_expr)

let expr_to_nodes (e : Analyzed_expr.expr) : Analyzed_expr_graph.node_record list =
  let gen_id =
  ...
  let children = [] in
  let group    = None in
  let rec aux (parent : string option) (e : Analyzed_expr.expr) : Analyzed_expr_graph.node_record list =
    let id      = gen_id () in
    ...
    | BinaryOp (op, l, r)  ->
      ...
      let right = aux (Some id) r in { id;
                                       node = Some (Data { expr_str = Expr_components.binary_op_to_string op; });
                                       parents  = Some parents;
                                       children = Some children;
                                       group;
                                       node_visible = Some true;
                                     ...
                                     :: (left @ right)
    | Quantified (q, _, e) -> { id;
                                node = Some (Data { expr_str = Expr_components.quantifier_to_string q; });
                                parents  = Some parents;
                                children = Some children;
                                group;
                                node_visible = Some true;
    ...
    | InsertedF l          -> [ { id;
                                  node = Some (Data { expr_str = Printf.sprintf "Inserted formula %s" (String.concat ", " (List.map string_of_int l)); });
                                  parents  = Some parents;
                                  children = Some children;
                                  group;
                                  node_visible = Some true;
...
let expr_to_node_map (e : Analyzed_expr.expr) : Analyzed_expr_graph.t =
  Analyzed_expr_graph.add_nodes_via_records Append (expr_to_nodes e) (Analyzed_expr_graph.empty)
...
module Analyzed_graph_base = struct
  ...
  let color_internal (_id : id) (node : node) : string =
    match node with
    | Data node -> (match node.classification with
        | Unsure                  -> "#FFC300"
        | Axiom                   -> "#74C2F6"
...
type info_source = Step of string
                 | Axiom of Analyzed_expr.expr
                 | Expr of Analyzed_expr.expr

type derive_explanation = Nothing_to_explain
                        | Dont_know_how
                        | Rewrite           of (Analyzed_expr.expr * Analyzed_expr.expr) list
                        | Combine_knowledge of info_source list * Analyzed_expr.expr list
                        | Gain_knowledge    of info_source list * (Analyzed_expr.expr * Analyzed_expr.expr) list * (Analyzed_expr.expr * Analyzed_expr.expr) list

type grouped_derive_explanations = Nothing_to_explain
                                 | Dont_know_how
                                 | Rewrites          of Analyzed_expr.expr list list
                                 | Combine_knowledge of info_source list * Analyzed_expr.expr list
                                 | Gain_knowledge    of info_source list * Analyzed_expr.expr list

let uniquify_universal_var_names_derive_explanations (explanations : derive_explanation list) : derive_explanation list =
  let replace_id (original : string) (replacement : string) (explanation : derive_explanation) : derive_explanation =
    let replace_id_for_pairs
        (original    : string)
        (replacement : string)
    ...
    | Rewrite pairs                             -> Rewrite (replace_id_for_pairs original replacement pairs)
    | Combine_knowledge (infos, es)             -> Combine_knowledge (List.map
                                                                        (fun info ->
                                                                           match info with
                                                                           | Expr e -> Expr (Analyzed_expr.replace_universal_var_name original replacement e)
                                                                           | _ -> info
                                                                        )
                                                                        infos,
                                                                      List.map (Analyzed_expr.replace_universal_var_name original replacement) es)
    | Gain_knowledge    (infos, pairs1, pairs2) -> Gain_knowledge (infos,
  ...
  let get_exprs (explanation : derive_explanation) : Analyzed_expr.expr list =
    let pairs_to_exprs (pairs : (Analyzed_expr.expr * Analyzed_expr.expr) list) : Analyzed_expr.expr list =
    ...
    | Combine_knowledge (infos, es)         -> (List.map
                                                  (fun info ->
                                                     match info with
                                                     | Expr e -> e
    ...
    | Gain_knowledge    (_, pairs1, pairs2) -> pairs_to_exprs pairs1 @
                                               pairs_to_exprs pairs2
  in
  Analyzed_expr.uniquify_universal_var_names_generic ~replace_id ~get_exprs "X" explanations

let var_bindings_in_explanations (explanations : derive_explanation list) : Analyzed_expr.expr Analyzed_expr.VarMap.t =
  ...
  List.fold_left
    (fun m e ->
       var_bindings_in_generic ~m ~get_pairs e
    )
    VarMap.empty
    explanations
...
let mark_free_variables (m : node_graph) : node_graph =
  ...
  let mark_free_variable_possibly () (id : id) (node : node) (m : node_graph) =
    ...
    let data = { data with
                 expr =
                   if parents = [] then
                     Analyzed_expr.mark_if_unsure Free data.expr
                   else
                     data.expr;
               } in
    ((), Analyzed_graph.add_node Overwrite id (Data data) m)
  in
  let ((), m) = Analyzed_graph.linear_traverse () (Full_traversal mark_free_variable_possibly) m in
  m

let mark_universal_variables (m : node_graph) : node_graph =
  let open Analyzed_graph in
  let mark_universal_variable_possibly () (id : id) (node : node) (m : node_graph) =
    let data = unwrap_data node in
    let data = { data with
                 expr = Analyzed_expr.mark_if_unsure Universal data.expr;
               } in
    ((), Analyzed_graph.add_node Overwrite id (Data data) m)
  in
  let ((), m) = Analyzed_graph.linear_traverse () (Full_traversal mark_universal_variable_possibly) m in
  m
...
let propagate_variable_boundedness (m : node_graph) : node_graph =
  ...
  let propagate_boundedness_to_child (m : node_graph) target_id child_id =
    let boundedness = Analyzed_expr.get_boundedness (unwrap_data (find_node target_id m)).expr in
    let node = unwrap_data (find_node child_id m) in
    let node = { node with
                 expr = Analyzed_expr.update_boundedness node.expr boundedness;
               } in
    Analyzed_graph.add_node Overwrite child_id (Data node) m
  in
  let propagate_boundedness_to_children () (id : id) (_node : node) (m : node_graph) =
    let children = find_children id m in
    ((),
     List.fold_left
       (fun m child_id ->
          propagate_boundedness_to_child m id child_id
       )
       m
       children)
  in
...
let classify_negated_goal (m : node_graph) : node_graph =
  ...
  let classify () (id : id) (node : node) (m : node_graph) : unit * node_graph =
    ...
    | Unsure ->
      let classification =
        if data.derive_descr = "negated_conjecture" then
          NegatedGoal
        else
          let parents = find_nodes (find_parents id m) m in
          let all_parents_negated_goal =
            List.for_all
              (fun x -> (unwrap_data x).classification = NegatedGoal) parents
          in
          if parents <> [] && all_parents_negated_goal then
            NegatedGoal
          else
            data.classification
      in
    ...
    | _ -> ((), m)
  in
  linear_traverse () (Full_traversal classify) m
  |> unwrap_tuple_1
    ...
    | Unsure ->
      let classification =
        ...
        let any_child_negated_goal =
          List.filter
            (fun x -> (unwrap_data x).classification = NegatedGoal) children <> []
        in
        if no_parents && any_child_negated_goal then
          Goal
        else
          data.classification
      in
        ...
        | [] ->
          let classification =
            ...
            | Function ("attacker",
              ...
              when check_tag tag -> ProtocolStep
            | Quantified (_,
                          _,
                          BinaryOp (Imply,
                                    _,
              ...
              when check_tag tag -> InteractiveProtocolStep
            | _ ->
              data.classification
          in
          let data = { data with classification } in
    ...
    | Unsure ->
      begin
        ...
        let all_parents_axiom_or_rewriting =
          List.for_all (fun x -> (unwrap_data x).classification = Axiom
                                 || (unwrap_data x).classification = Rewriting)
            parents
        in
        let classification =
          if parents <> [] && all_parents_axiom_or_rewriting then
            Rewriting
          else
            data.classification
        in
    ...
    | Unsure ->
      begin
        ...
        let any_parents_protocol_step_or_knowledge =
          List.exists (fun x -> (unwrap_data x).classification = InteractiveProtocolStep
        ...
        in
        let classification =
          if any_parents_protocol_step_or_knowledge then
            Knowledge
          else
            data.classification
        in
      ...
      begin
        ...
        | parent_ids ->
          ...
          let any_parent_alias =
            List.exists
              (fun x -> (unwrap_data x).classification = Alias) parents
          in
          let classification =
            if any_parent_alias then
              let aliases =
                List.filter
                  (fun id -> (unwrap_data (find_node id m)).classification = Alias) parent_ids
              in
              ...
              let children =
                List.filter
                  (fun x -> x <> id) (find_children alias m)
              in
              match children with
              | []       -> data.classification
              | children ->
...
let redraw_alias_arrows (m : node_graph) : node_graph =
  ...
  let nodes = to_assoc_list (
      filter (fun id _node m ->
          let parents = find_nodes (find_parents id m) m in
          let alias_count = List.length (List.filter (fun n -> (unwrap_data n).classification = Alias) parents) in
          alias_count > 1
        ) m
    )
  in
  ...
  (* redraw arrows *)
  List.fold_left (fun m (id, n) ->
      let parents = find_parents id m in
      let connected_aliases = List.filter (fun id -> (unwrap_data (find_node id m)).classification = Alias) parents in
      let parents = List.filter (fun id -> not (List.mem id connected_aliases)) parents in
...
let rewrite_conclusion (m : node_graph) : node_graph =
  ...
  match (filter_first_opt
           (fun _ node _ -> (unwrap_data node).classification = Goal) m) with
  | None -> m
  | Some goal_id ->
    let goal = find_node goal_id m in

    let remove_ids =
      collect_ids_to_remove goal_id m
    in

    let exclude_parent_ids =
      goal_id :: remove_ids
    in
    ...
    let parents =
      List.filter
        ...
        (List.fold_left
           (fun acc id -> find_parents id m @ acc)
           []
           remove_ids)
    in
...
let classify_nodes (m : node_graph) : node_graph =
  ...
  let classify_node () (id : id) (node : node) (m : node_graph) =
    ...
    let new_node =
      ...
      | [] ->
        ...
        let is_axiom        = match data.expr with
          | Analyzed_expr.Quantified _ -> true
          | _                          -> false in
        let is_knowledge_possibly = List.filter (fun x -> x = "attacker") func_names <> [] in
        { data with
          classification =
      ...
      | parents ->
        ...
        let any_parent_negated_goal =
          List.filter
            (fun (x : node) -> (unwrap_data x).classification = NegatedGoal) parents <> [] in
        ...
        let any_parent_knowledge =
          List.filter
            (fun (x : node) ->
               let data = unwrap_data x in
               data.classification = InitialKnowledge || data.classification = Knowledge)
            parents <> [] in
        { data with
          classification =
  ...
  in
  let ((), m) = Analyzed_graph.linear_traverse () (Full_traversal classify_node) m in
  m
...
let get_chain (m : node_graph) (id : string) : string * string * (string list) =
  let open Analyzed_graph in
  let rec aux (m : node_graph) (id : string) (acc : string list) : string * string * (string list) =
    let node     = find_node id m in
    let data     = unwrap_data node in
    let parents  = find_parents id m in
    let children = find_children id m in
    let acc =
      if (parents = [] || List.length parents = 1)
      && data.classification <> Contradiction then
        id :: acc
      else
        acc
    in
...
let mark_chains (m : node_graph) : node_graph =
  ...
  let rec update_chain (m : node_graph) chain ids =
    ...
    | x :: xs ->
      ...
      update_chain (Analyzed_graph.add_node Overwrite x (Data data) m) chain xs
  in
  let mark_chain () (id : id) (_node : node) (m : node_graph) =
    ...
    |  _ -> ((), m)
  in
  linear_traverse () (Full_traversal mark_chain) m
  |> Misc_utils.unwrap_tuple_1
...
let compress_rewrite_trees (m : node_graph) : node_graph =
  ...
  let (bottom_id, _) =
    List.hd
      (to_assoc_list
         (filter (fun id _n m -> find_children id m = []) m))
  in
...
let node_list_to_map (node_records : Analyzed_graph.node_record list) : node_graph =
  let classify (m : node_graph) : node_graph =
    m
    |> classify_negated_goal
    |> classify_goal
    |> classify_protocol_step
    |> classify_axiom
    |> classify_rewriting
    |> classify_initial_knowledge
    |> classify_contradiction
    |> classify_knowledge
  in
  Analyzed_graph.add_nodes_via_records Overwrite node_records Analyzed_graph.empty
  |> mark_free_variables
  |> propagate_variable_boundedness
  |> mark_universal_variables
  |> classify
  |> classify_alias
  |> classify
  |> rewrite_conclusion
  ...
  let rec aux (id : id) (m : node_graph) : info_source list =
    ...
    | [] -> (
        ...
        | Function ("attacker",
          ...
          when check_tag tag -> [Step (reformat_tag tag)]
        | Quantified (_,
                      _,
                      BinaryOp (Imply,
                                _,
...
let explain_construction_single (id : Analyzed_graph.id) (m : node_graph) : derive_explanation =
  let open Analyzed_graph in
  let open Analyzed_expr  in

  let is_rewrite (e : expr) : bool =
    match e with
    | BinaryOp (Eq, _, _) -> true
    | _                   -> false
  in
      ...
      | [agent_id] ->
        begin
          ...
          let base_expr   = base_expr
                            |> Analyzed_expr.negate
                            |> strip_quant
          in
          let agent_expr  = agent_expr
                            |> Analyzed_expr.negate
                            |> strip_quant
          in
          ...
          in
          let agent_exprs  = agent_exprs
                             |> List.map strip_not
                             |> List.map strip_pred_attacker
                             |> List.filter (fun x -> x <> False)
          in
          let result_exprs = result_exprs
                             |> List.map strip_not
                             |> List.map strip_pred_attacker
                             |> List.filter (fun x -> x <> False)
          in
          ...
          | [b_expr], [a_expr], [r_expr] when is_rewrite a_expr -> Rewrite [(r_expr, b_expr)]
          | [b_expr], _       , r_exprs  ->
            Combine_knowledge ((trace_sources agent_id m) @ (List.map (fun x -> Expr x) r_exprs),
                               [b_expr]
                              )
          | b_exprs , [a_expr], r_exprs
            ...
            Js_utils.console_log (Printf.sprintf "match_map size : %d" (List.length (ExprMap.bindings (unwrap_opt match_map))));
            Rewrite (ExprMap.bindings (unwrap_opt match_map))
          | b_exprs , a_exprs , r_exprs
            when List.length b_exprs = (List.length a_exprs + List.length r_exprs)->
            let match_map      = unwrap_opt (pattern_multi_match_map (a_exprs @ r_exprs) b_exprs) in
            let new_knowledge =
              ExprMap.bindings match_map
              |> List.filter
                (fun (k, _) -> List.mem k a_exprs)
            in
            let old_knowledge =
              ExprMap.bindings match_map
              |> List.filter
                (fun (k, _) -> List.mem k r_exprs)
            in
            Gain_knowledge (trace_sources agent_id m,
                            new_knowledge,
                            old_knowledge
                           )
          | _                            -> Dont_know_how
...
let explain_construction_single_chained (id : Analyzed_graph.id) (m : node_graph) : derive_explanation list =
  let open Analyzed_graph in
  let open Analyzed_expr in
  ...
  List.map
    (fun id ->
       explain_construction_single id m
    )
    (get_chain id m)

let group_derive_explanations (explanations : derive_explanation list) : grouped_derive_explanations list =
  let open Analyzed_expr in
  let same_group (e1 : derive_explanation) (e2 : derive_explanation) : bool =
    ...
    | _                                        -> false
  in
  let group_into_list (explanations : derive_explanation list) : derive_explanation list list =
    let rec aux (acc : derive_explanation list list) (explanations : derive_explanation list) : derive_explanation list list =
      ...
      | e :: es ->
        let acc : derive_explanation list list =
          ...
          | last_es :: acc ->
            ...
            | []                -> [e] :: acc
            | last_e :: last_es ->
              if same_group e last_e then
                (e :: last_e :: last_es) :: acc
              else
                [e] :: (last_e :: last_es) :: acc
        in
        aux acc es
    ...
    aux [] explanations
  in
  let convert_rewrite (explanations : derive_explanation list) : grouped_derive_explanations =
    let rec match_into_buckets (buckets : expr list list) (pairs_list : (expr * expr) list list) : expr list list =
      ...
      | pairs :: rest ->
        let buckets =
          ...
          | buckets ->
            ...
            let match_map    = unwrap_opt (pattern_multi_match_map bucket_heads pair_heads) in
            List.map
              (fun l ->
                 match l with
                 | []       -> []
                 | hd :: tl ->
                   let new_hd = List.assoc (ExprMap.find hd match_map) pairs in
                   new_hd :: hd :: tl
              )
              buckets
        in
  ...
  in
  let convert_combine_knowledge (explanations : derive_explanation list) : grouped_derive_explanations list =
    List.map
      (fun (e : derive_explanation) : grouped_derive_explanations ->
         match e with
      ...
      explanations
  in
  let convert_gain_knowledge (explanations : derive_explanation list) : grouped_derive_explanations list =
    List.map
      (fun (e : derive_explanation) : grouped_derive_explanations ->
         match e with
         | Gain_knowledge (info_src, new_knowledge, _) -> Gain_knowledge (info_src, List.map (fun (k, _) -> k) new_knowledge)
         | _                                           -> failwith "Incorrect case"
      )
      explanations
  in
  let convert_to_grouped (grouped_explanations : derive_explanation list list) : grouped_derive_explanations list =
    let rec aux (acc : grouped_derive_explanations list) (grouped_explanations : derive_explanation list list) : grouped_derive_explanations list =
  ...
  let explanations =
    explanations
    |> uniquify_universal_var_names_derive_explanations
    |> List.filter (fun (x : derive_explanation) -> x <> Nothing_to_explain && x <> Dont_know_how)
  in
...
let explain_construction_chain (id : Analyzed_graph.id) (m : node_graph) : grouped_derive_explanations list =
  let open Analyzed_graph in
  let open Analyzed_expr  in
...
let derive_explanation_to_string (explanation : derive_explanation) : string =
  ...
  | Dont_know_how                     -> "N/A"
  | Rewrite rewrites                  -> Printf.sprintf
                                          "Attacker rewrites
%s
                                          "
                                          (String.concat "\n\n"
...
to
  %s"
                                                     (expr_to_string e_from)
                                                     (expr_to_string e_to)
                                                )
                                                rewrites
                                             )
                                          )
  | Combine_knowledge (srcs_from, es_gain) -> Printf.sprintf
                                                "From
%s
attacker knows
%s
  ...
  | Gain_knowledge (srcs_from, new_knowledge, old_knowledge) -> Printf.sprintf
                                                                  "From
%s
attacker learns
%s
...
let grouped_derive_explanations_to_string (explanation : grouped_derive_explanations) : string =
  let open Analyzed_expr in
  match explanation with
  | Nothing_to_explain                -> "Nothing to explain"
  | Dont_know_how                     -> "Don't know how to explain"
  | Rewrites buckets                  -> Printf.sprintf
                                           "%s
"
                                           (String.concat "\n\n"
                                              (List.map
...
%s
  ...
  | Gain_knowledge (srcs_from, es_gain) -> Printf.sprintf
                                                "From
%s
attacker knows
%s
...
let grouped_derive_explanations_list_to_string (explanations : grouped_derive_explanations list) : string =
  String.concat "\n"
    (List.map
       grouped_derive_explanations_to_string
       explanations
    )

let node_to_string_debug (id : Analyzed_graph.id) (node : node) (m : node_graph) : string =
  let open Analyzed_graph in
  match node with
  | Group     -> "Group node"
  | Data data ->
   ...
   [chain          : %s->%s]"
      ...
      (Misc_utils.join_with_comma (Analyzed_expr.get_function_names data.expr))
      (Misc_utils.join_with_comma parents)
      (Misc_utils.join_with_comma children)
      (let (hd, _, _) = data.chain in hd)
      (let (_, tl, _) = data.chain in tl)

let write_map_debug (out : out_channel) (m : node_graph) : unit =
  let open Analyzed_graph in
  linear_traverse
    ()
    (Full_traversal_pure (fun () id node m -> Printf.fprintf out "%s\n" (node_to_string_debug id node m)))
    m
  |> ignore

let map_debug_string (m : node_graph) : string =
  let open Analyzed_graph in
  let (acc, _) = linear_traverse
      []
      (Full_traversal_pure (fun acc id node m -> (node_to_string_debug id node m) :: acc))
      m
  in
  String.concat "\n" (List.rev acc)
...
let print_map_debug (m : node_graph) : unit =
  ...
  linear_traverse
    ()
    (Full_traversal_pure (fun () id node m ->
      print_endline (node_to_string_debug id node m)))
    m
  |> ignore
...
let write_map_DAG (out : out_channel) (m : node_graph) : unit =
  let open Printf in
  let open Analyzed_graph in
  let write_node (out : out_channel) (id : id) (node : node) (m : node_graph) : unit =
    let children = find_children id m in
    List.iter
      (fun x -> fprintf out "%s --> %s;\n" id x)
      children;
    ...
    if head <> tail then
      fprintf out "%s->%s" head tail
    else
      fprintf out "%s" id;
    fprintf out ". ";
    fprintf out "%s <br>" (Analyzed_expr.expr_to_string data.expr);
    fprintf out "%s"     (String.capitalize_ascii (classification_to_string data.classification));
  ...
  linear_traverse
    ()
    (Full_traversal_pure
       (fun () id node m -> write_node out id node m))
    m
  |> ignore

let print_map_DAG (m : node_graph) : unit =
  write_map_DAG stdout m
...
let collect_steps (m : node_graph) : Protocol_step.t list =
  ...
  let steps_reachable (path : id list) (m : node_graph) : Protocol_step.t list =
     ...
     *   aux [] id m
     * in *)
    let find_steps_at_top (acc : Protocol_step.t list) (id : id) (_node : node) (m : node_graph) =
      match find_parents id m with
      | [] -> (* reached top, check classification *)
        let node = find_node id m in
        let classification = (unwrap_data node).classification in
      ...
      | _ -> acc
    in
    let rec aux (acc : Protocol_step.t list) (last_id : id option) (path : id list) (m : node_graph) =
      match path with
      | [] -> List.rev acc
      | x :: xs ->
        (* check if any other path upward (i.e. a turn) to take *)
        let choices_upward = List.filter
            (fun x ->
               match last_id with
               | None -> true
        ...
        let acc = List.fold_left
            (fun acc parent ->
               let (r, _) = bfs_traverse parent acc Bottom_to_top (Full_traversal_pure find_steps_at_top) m in
               r
            )
            acc
            choices_upward
  ...
  let steps = List.fold_left
      (fun acc l ->
         Js_utils.console_log (Printf.sprintf "list : [%s]" (String.concat ", " (List.map (fun x -> Printf.sprintf "%s.%d" (match x.proc_name with None -> "" | Some x -> x) x.step_num) l)));
         Misc_utils.zip l acc
      )
      []
      (List.map (fun path -> steps_reachable path m) paths)
  in
  let steps = List.fold_left (* if there are multiple occurances of a step, only keep the later ones *)
      (fun acc s ->
   ...
   *     immediate_roots
   * in *)
  let grouped_steps = Misc_utils.group_list (fun s1 s2 -> s1.proc_name = s2.proc_name) steps in
  List.concat
    (List.map
       (fun l ->
          List.sort (fun s1 s2 -> compare s1.step_num s2.step_num) l
       )
       grouped_steps
    )
