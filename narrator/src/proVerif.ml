...
module Raw_expr = struct
  ...
  let rec expr_to_string f =
    match f with
    | Variable n -> n
    | Function (name, fs) -> Printf.sprintf "%s(%s)" name (String.concat ", " (List.map expr_to_string fs))
    | UnaryOp (op, f) ->
      Printf.sprintf "%s%s"
        ...
        (match f with
         | Variable _
         | Function _
         | UnaryOp _ -> expr_to_string f
         | _ -> Printf.sprintf "(%s)" (expr_to_string f))
    | BinaryOp (op, l, r) ->
      Printf.sprintf "%s %s %s"
        (match l with
         | Variable _
         | Function _
         | UnaryOp _ -> expr_to_string l
         | _ -> Printf.sprintf "(%s)" (expr_to_string l))
        (binary_op_to_string op)
        (match r with
         | Variable _
         | Function _
         | UnaryOp _ -> expr_to_string r
         | _ -> Printf.sprintf "(%s)" (expr_to_string r))
    | Tuple fs -> Printf.sprintf "(%s)" (String.concat ", " (List.map expr_to_string fs))

  module Expr_parser = struct
    let parens (p : expr stateless_p) : expr stateless_p =
      ignore_space (skip_char '(') >> p >>= (fun x -> ignore_space (skip_char ')') >> return x)
    ...
    and
      func_p s = (ignore_space
                    (ident_p >>= (fun ident -> char '(' >>
                                    sep_by1 expr_p (char ',') >>=
                                    (fun params -> char ')' >>
                                      return (Function (ident, params)))
                                  ))) s
    and
      tuple_p s = (ignore_space
                     (skip_char '(') >> (sep_by1 expr_p (char ',')) >>=
                   (fun fs ->
    ...
    and
      sub_expr_p s = choice [ attempt func_p
                            ; attempt atom_p
                            ; attempt tuple_p
                            ] s
    and
  ...
  type proc_instance_ty = Single | Multi
  ...
  module Print_context = struct
    ...
    let insert_blank_line_anyway_proc_struct_ty ctx =
      match ctx.prev_proc_struct_ty with
      | Some _ -> push ctx ""
      | None -> ()
  ...
  let rec pv_proc_expr_to_string_debug e =
    match e with
    | Let (args, e) -> (match args with
        | [Ident ident] -> Printf.sprintf "let %s = %s" ident (expr_to_string e)
        | _ -> Printf.sprintf "let (%s) = %s" (String.concat ", " (List.map
                                                                     (function
                                                                       | Ident i -> i
                                                                       | Ident_ty (i, ty) -> Printf.sprintf "%s : %s" i ty
                                                                       | Pat e -> Printf.sprintf "=%s" (expr_to_string e)) args)) (expr_to_string e)
      )
    | IfElse x -> (
        let cond = expr_to_string x.condition in
        let true_branch = String.concat "\n" (List.map pv_proc_expr_to_string_debug x.true_branch) in
        let false_branch = String.concat "\n" (List.map pv_proc_expr_to_string_debug x.false_branch) in
        match x.false_branch with
        | [] -> Printf.sprintf "if %s then\n%s" cond true_branch
        | _  -> Printf.sprintf "if %s then\n%s\nelse\n%s" cond true_branch false_branch
      )
    | In(ident, args) -> Printf.sprintf "in(%s, %s);" ident (String.concat ", " (List.map
                                                                                   (fun (e : ident_ty_or_pat) ->
                                                                                      match e with
                                                                                      | Ident_ty (i, ty) -> Printf.sprintf "%s:%s" i ty
                                                                                      | Pat e -> Printf.sprintf "=%s" (expr_to_string e)) args))
    | Out(ident, e) -> Printf.sprintf "out(%s, %s);" ident (expr_to_string e)
    | Zero -> "0"
    | New(ident, ty) -> Printf.sprintf "new %s:%s" ident ty
    | Get (ident, args) -> (Printf.sprintf "get %s(%s)" ident (String.concat ", " (List.map
                                                                                     (function
                                                                                       | Ident i -> i
                                                                                       | Ident_ty (i, ty) -> Printf.sprintf "%s : %s" i ty
                                                                                       | Pat e -> Printf.sprintf "=%s" (expr_to_string e)) args))
                           )
    | Insert (ident, args) -> (Printf.sprintf "insert %s(%s)" ident (String.concat ", " (List.map
                                                                                           (function
                                                                                             | Ident i -> i
                                                                                             | Ident_ty (i, ty) -> Printf.sprintf "%s : %s" i ty
                                                                                             | Pat e -> Printf.sprintf "=%s" (expr_to_string e)) args))
  ...
  let rec pv_expr_to_string_debug e =
    match e with
    | Const(ident, ty, v) -> Printf.sprintf "const %s : %s%s." ident ty (match v with | Public -> "" | Private -> " [private]")
    | Free(ident, ty, v) -> Printf.sprintf "free %s : %s%s." ident ty (match v with | Public -> "" | Private -> " [private]")
    | Table(ident, tys) -> Printf.sprintf "table %s(%s)." ident (String.concat ", " tys)
    | FunDec(ident, tys, ret_ty) -> Printf.sprintf "fun %s(%s) : %s." ident (String.concat ", " tys) ret_ty
    | Equation eq -> Printf.sprintf "equation forall %s, %s." (String.concat " "
                                                                 (List.map (fun (ident, ty) ->
                                                                      Printf.sprintf "%s:%s" ident ty)
                                                                     eq.vars)) (expr_to_string eq.formula)
    | LetProc(ident, args, es) -> (
        match args with
        | [] -> Printf.sprintf "let %s =\n%s" ident (pv_proc_exprs_to_string_debug es)
        | _ -> Printf.sprintf "let %s(%s) =\n%s" ident (String.concat ", "
                                                          (List.map (fun (ident, ty) ->
                                                               Printf.sprintf "%s : %s" ident ty)
                                                              args)) (pv_proc_exprs_to_string_debug es)
      )
    (* | FunApp(ident, es) -> Printf.sprintf "%s(%s)" ident (String.concat ", " (List.map aux es)) *)
    | Process (proc, ps) -> Printf.sprintf "process %s%s"
                              (pv_proc_exprs_to_string_debug proc)
                              (String.concat " | "
                                 (List.map
                                    (fun (instance_ty, p) ->
                                       match p with
                                       | Name (name, args) -> (match instance_ty with
                                           | Single -> (match args with
                                               | [] -> name
                                               | _ -> Printf.sprintf "%s(%s)" name (String.concat ", " args)
                                             )
                                           | Multi -> Printf.sprintf "(! %s)" (match args with
                                               | [] -> name
                                               | _ -> Printf.sprintf "%s(%s)" name (String.concat ", " args)
                                             )
                                         )
                                       | Proc p -> (match instance_ty with
                                           | Single ->
    ...
    | Query(e) -> Printf.sprintf "query %s." (expr_to_string e)
  ...
  let pv_proc_expr_to_string ctx e =
    let rec aux e =
      ...
      | In(_, args) -> (
          ...
          Print_context.insert_blank_line_if_diff_proc_struct_ty ctx;
          let proc_name   = match ctx.proc_name with
            | Some name -> name
            | None -> ""
          in
          let padding_len = match ctx.proc_name with
            | Some name -> String.length name + 2 + 4
            | None      -> 0
          in
          let padding = String.make padding_len ' ' in
          Print_context.push ctx (Printf.sprintf "%sI -> %s : IN (%s)"
                                    padding
                                    proc_name
      ...
      | Out(_, e) -> (
          ...
          let proc_name = match ctx.proc_name with
            ...
            | None -> ""
          in
          let proc_step_tag = match ctx.proc_name with
            | Some name -> Printf.sprintf "%s.%d" name ctx.out_count
            | None -> ""
          in
          Print_context.push ctx (Printf.sprintf "%s    %s -> I : OUT(%s)" proc_step_tag proc_name (expr_to_string e))
        )
      | Zero -> (
      ...
      | Get(ident, args) -> (
          ...
          Print_context.insert_blank_line_if_diff_proc_struct_ty ctx;
          Print_context.push ctx (Printf.sprintf "get %s(%s)" ident (String.concat ", " (List.map
                                                                                           (function
                                                                                             | Ident i -> i
                                                                                             | Ident_ty (i, ty) -> Printf.sprintf "%s : %s" i ty
                                                                                             | Pat e -> Printf.sprintf "=%s" (expr_to_string e)) args))
      ...
      | Insert(ident, args) -> (
          Print_context.set_proc_struct_ty ctx Print_context.Get;
          Print_context.insert_blank_line_if_diff_proc_struct_ty ctx;
          Print_context.push ctx (Printf.sprintf "insert %s(%s)" ident (String.concat ", " (List.map
                                                                                              (function
                                                                                                | Ident i -> i
                                                                                                | Ident_ty (i, ty) -> Printf.sprintf "%s : %s" i ty
                                                                                                | Pat e -> Printf.sprintf "=%s" (expr_to_string e)) args))
  ...
  let pv_expr_to_string ?(ctx = Print_context.make ()) e =
    let aux e =
      ...
      | Const(ident, ty, v) -> (
          ...
          Print_context.push ctx (Printf.sprintf "const %s : %s%s." ident ty (match v with | Public -> "" | Private -> " [private]"))
        )
      | Free(ident, ty, v) -> (
          Print_context.set_struct_ty ctx Print_context.Free;
          Print_context.insert_blank_line_if_diff_struct_ty ctx;
          Print_context.push ctx (Printf.sprintf "free %s : %s%s." ident ty (match v with | Public -> "" | Private -> " [private]"))
        )
      | Table(ident, tys) -> (
          Print_context.set_struct_ty ctx Print_context.Table;
          Print_context.insert_blank_line_if_diff_struct_ty ctx;
          Print_context.push ctx (Printf.sprintf "table %s(%s)." ident (String.concat ", " tys))
        )
      | FunDec(ident, tys, ret_ty) -> (
          Print_context.set_struct_ty ctx Print_context.FunDec;
          Print_context.insert_blank_line_if_diff_struct_ty ctx;
          Print_context.push ctx (Printf.sprintf "fun %s(%s) : %s." ident (String.concat ", " tys) ret_ty)
        )
      | Equation eq -> (
          Print_context.set_struct_ty ctx Print_context.Equation;
          Print_context.insert_blank_line_if_diff_struct_ty ctx;
          Print_context.push ctx (Printf.sprintf "equation forall %s, %s."
                                    (String.concat " " (List.map
                                                          (fun (ident, ty) ->
                                                             Printf.sprintf "%s:%s" ident ty)
                                                          eq.vars))
      ...
      | LetProc(ident, args, es) -> (
          ...
          (match args with
           ...
           | _ -> Print_context.push ctx (Printf.sprintf "let %s(%s) =" ident (String.concat ", "
                                                                                 (List.map (fun (ident, ty) ->
                                                                                      Printf.sprintf "%s : %s" ident ty) args)))
          );
          Print_context.incre_indent ctx;
            ...
            (fun (instance_ty, p) ->
               ...
               | Name (name, args) -> (match instance_ty with
                   | Single -> (match args with
                       ...
                       | _ ->
                         Print_context.push ctx (Printf.sprintf "| %s(%s)" name (String.concat ", " args))
                     )
                   | Multi -> (match args with
                       | [] ->
                         Print_context.push ctx (Printf.sprintf "| (! %s)" name)
                       | _ ->
                         Print_context.push ctx (Printf.sprintf "| (! %s(%s))" name (String.concat ", " args))
      ...
      | Query(e) -> (
          ...
          Print_context.insert_blank_line_if_diff_struct_ty ctx;
          Print_context.set_struct_ty ctx Print_context.Query;
          Print_context.push ctx (Printf.sprintf "query %s." (expr_to_string e))
        )
    in
    aux e
  ...
  module Parser = struct
    ...
    let const_p = (ignore_space
                     (skip_string "const" >> spaces >> ident_p >>=
                      (fun ident ->
                         spaces >> char ':' >> spaces >> ident_p >>=
                         (fun ty ->
                            (ignore_space (skip_char '[') >> ignore_space (skip_string "private") >> ignore_space (skip_char ']') >>
                             skip_char '.' >>
                             return (Free (ident, ty, Private)))
                            <|>
                            (skip_char '.' >>
                             return (Free (ident, ty, Public)))
                         )
                      )))

    let free_p = (ignore_space
    ...
    let args_p = many1 (arg_p >>= (fun arg -> optional (skip_char ',') >> return arg))

    let ident_ty_p = (ignore_space ident_p >>=
                      (fun ident ->
                         ignore_space (skip_char ':') >> ident_p >>=
                         (fun ty ->
                            return (ident, ty)
                         )
                      ))

    let ident_or_ident_ty_or_pat_p = (ignore_space
                                        (
                                          (attempt (ident_ty_p >>= (fun (i, ty) -> return (Ident_ty (i, ty) : ident_or_ident_ty_or_pat))))
                                          <|>
                                          (ident_p >>= (fun x -> return (Ident x)))
                                          <|>
                                          (skip_char '=' >> Expr_parser.expr_p >>= (fun e -> return (Pat e : ident_or_ident_ty_or_pat)))
                                        )
                                     )

    let ident_ty_or_pat_p = (ignore_space
                               (
                                 (ident_ty_p >>= (fun (i, ty) -> return (Ident_ty (i, ty) : ident_ty_or_pat)))
                                 <|>
                                 (skip_char '=' >> Expr_parser.expr_p >>= (fun e -> return (Pat e : ident_ty_or_pat)))
                               )
                            )

    let fun_dec_p = (ignore_space
                       (skip_string "fun" >> ignore_space ident_p >>=
                        (fun ident ->
                           skip_char '(' >> args_p >>=
                           (fun tys ->
                              skip_char ')' >> ignore_space (skip_char ':') >>
                              ignore_space ident_p >>=
                              (fun ret_ty ->
                                 skip_char '.' >>
                                 return (FunDec (ident, tys, ret_ty))
                              )
                           )
                        )))

    let table_p = (ignore_space
                     (skip_string "table" >> ignore_space ident_p >>=
                      (fun ident ->
                         skip_char '(' >> args_p >>=
                         (fun tys ->
                            ignore_space (skip_char ')') >> skip_char '.' >>
                            return (Table (ident, tys))
                         )
                      )))

    let eq_p = (ignore_space
    ...
    let query_p = (ignore_space
                     (skip_string "query" >> spaces >>
                      Expr_parser.expr_p >>=
                      (fun f ->
                         skip_char '.' >>
                         return (Query f)
    ...
    let get_p = (ignore_space
                   (skip_string "get") >> ignore_space ident_p >>=
                 (fun ident ->
                    ignore_space (skip_char '(') >> sep_by ident_or_ident_ty_or_pat_p (ignore_space (skip_char ',')) >>=
                    (fun args ->
                       ignore_space (skip_char ')') >> ignore_space (skip_string "in") >>
                       return (Get (ident, args))
                    )
                 )
                )

    let insert_p = (ignore_space
                   (skip_string "insert") >> ignore_space ident_p >>=
                 (fun ident ->
                    ignore_space (skip_char '(') >> sep_by ident_or_ident_ty_or_pat_p (ignore_space (skip_char ',')) >>=
                    (fun args ->
                       ignore_space (skip_char ')') >> ignore_space (optional (skip_string ";")) >>
                       return (Insert (ident, args))
                    )
                 )
                )

    let rec let_p s = (ignore_space
                         (skip_string "let") >>
                       (
                         (ignore_space ident_p >>=
                          (fun ident ->
                             ignore_space (skip_char '=') >> Expr_parser.expr_p >>=
                             (fun f ->
                         ...
                         (ignore_space (skip_char '(') >> sep_by ident_or_ident_ty_or_pat_p (ignore_space (skip_char ',')) >>=
                          (fun args ->
                             ignore_space (skip_char ')') >> ignore_space (skip_char '=') >> Expr_parser.expr_p >>=
                             (fun f ->
                                ignore_space (skip_string "in") >>
                                return (Let (args, f))
                             )
    ...
    and if_else_p s = (ignore_space
                         (skip_string "if") >> ignore_space Expr_parser.expr_p >>=
                       (fun condition ->
                          ignore_space (skip_string "then") >>
                          ignore_space (opt_parens proc_exprs_p) >>=
                          (fun true_branch ->
                             (ignore_space (skip_string "else") >>
                              opt_parens (proc_exprs_p >>=
                                          (fun false_branch ->
    ...
    and in_p = (ignore_space
                  (skip_string "in") >> ignore_space (skip_char '(' >> ignore_space ident_p >>=
                                                      (fun ident ->
                                                         ignore_space (skip_char ',') >>
                                                         ((parens (sep_by1 ident_ty_or_pat_p (ignore_space (skip_char ',')) >>=
                                                                   (fun args ->
                                                                      return (In (ident, args))
                                                                   )
                                                                  ))
                                                          <|>
                                                          (ident_ty_p >>=
                                                         ...
                                                         >>=
                                                         (fun r ->
                                                            ignore_space (skip_char ')') >> ignore_space (optional (skip_char ';')) >>
                                                            return r
                                                         )
                                                      )))
    and out_p = (ignore_space
                   (skip_string "out") >> ignore_space (skip_char '(' >> ignore_space ident_p >>=
                                                        (fun ident ->
                                                           ignore_space (skip_char ',') >> Expr_parser.expr_p >>=
                                                           (fun e ->
                                                              ignore_space (skip_char ')') >> ignore_space (optional (skip_char ';')) >>
    ...
    and new_p = (ignore_space (skip_string "new") >> ignore_space (ident_ty_p) >>=
                 (fun (ident, ty) ->
                    ignore_space (optional (skip_char ';')) >>
                    return (New(ident, ty))
                 )
                )
    and proc_expr_p s = choice [ attempt let_p
                               ; attempt if_else_p
                               ; attempt in_p
                               ; attempt out_p
                               ; attempt zero_p
    ...
    let let_proc_p = (ignore_space
                        (skip_string "let") >> ignore_space ident_p >>=
                      (fun ident ->
                         (
                           ignore_space (skip_char '(') >> sep_by ident_ty_p (ignore_space (skip_char ',')) >>=
                           (fun args ->
                              ignore_space (skip_char ')') >> ignore_space (skip_char '=') >>
                              proc_exprs_p >>=
                              (fun es ->
                                 ignore_space (skip_char '.') >>
                                 return (LetProc (ident, args, es))
                              )
                           )
                         )
                         <|>
                         (
                           ignore_space (skip_char '=') >> proc_exprs_p >>=
                           (fun es ->
                              ignore_space (skip_char '.') >>
                              return (LetProc (ident, [], es))
                           )
                         )
                      )
                     )

    let proc_p = (ignore_space
                    (skip_string "process") >>
                  (proc_exprs_p >>=
                   (fun proc ->
                      (sep_by1 (
                          ignore_space (
                            (attempt (parens (
                              ignore_space (skip_char '!') >>
                              (choice [ (parens proc_exprs_p >>=
                                         (fun es ->
                                            return (Multi, (Proc es))
                                         ))
                                      ; attempt (ident_p >>=
                                         (fun ident ->
                                            ignore_space (skip_char '(') >> sep_by ident_p (ignore_space (skip_char ',')) >>=
                                            (fun args ->
                                               ignore_space (skip_char ')') >>
                      ...
                      (fun ps ->
                         return (Process (proc, ps))
                      )
                   )
                  )
                 )

    let expr_p = choice [ const_p
                        ; free_p
                        ; table_p
                        ; fun_dec_p
                        ; query_p
    ...
    let comment_p = (skip_string "(*") >> many_until skip_any_char_or_nl (skip_string "*)") >> return []

    let not_comment_p = many_until any_char_or_nl (look_ahead (skip_string "(*") <|> eof)
...
let remove_comments s =
  ...
  | Success l -> Ok (Core_kernel.String.of_char_list (List.concat l))
  | Failed(m, _) -> Error m

let process_string s =
  match remove_comments s with
  | Error e -> Error e
  | Ok s -> match MParser.parse_string Raw_expr.Parser.exprs_p s () with
    | Success ps -> Ok (Raw_expr.pv_exprs_to_string ps)
    | Failed(s, _) -> Error s
...
let () =
  ...
  | Ok s -> Js_utils.console_log s
  | Error e -> Js_utils.console_log e
