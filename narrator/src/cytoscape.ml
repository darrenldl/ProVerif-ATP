...
type node_shape = Circle | Ellipse | Diamond | Rectangle

class type node_data = object
...
class type t = object
  ...
  method on                  : Js.js_string Js.t -> Js.js_string Js.t -> (event Js.t -> unit) -> unit Js.meth
  method on_noselect         : Js.js_string Js.t -> (event Js.t -> unit) -> unit Js.meth
...
let make (container_id : string) : t Js.t =
  let container = Js.Unsafe.global##.document##getElementById (Js.string container_id) in
...
let zoom (cy : t Js.t) (level : float) : unit =
  cy##zoom level

let zoom_get (cy : t Js.t) : float =
  cy##zoom_get
...
let clear
    (cy : t Js.t)
  : unit =
  cy##remove_node (Js.string "node")

let on
    ?(selector : string option)
    (cy : t Js.t)
    ~(event    : string)
    ~(handler  : event Js.t -> unit)
  : unit =
...
let set_node_label
    ...
    ~(label : string)
  : unit =
  (cy##style##selector (Js.string (Printf.sprintf "node[id=\"%s\"]" id)))
   ##style (Js.string "label") (Js.string label) |> ignore;
...
let set_node_color
  ...
  (match id with
   | None    ->
     (cy##style##selector (Js.string "node"))
     ##style (Js.string "background-color") (Js.string color)
   | Some id ->
     (cy##style##selector (Js.string (Printf.sprintf "node[id=\"%s\"]" id)))
     ##style (Js.string "background-color") (Js.string color)
  ) |> ignore;
...
let set_node_size
  ...
  ((cy##style##selector (Js.string (Printf.sprintf "node[id=\"%s\"]" id)))
    ##style (Js.string "width") size)
   ##style (Js.string "height") size |> ignore;
  if update_style then
    style_update cy
  else
    ()

let set_node_position
    (cy  : t Js.t)
    ~(id : string)
    ~(x  : int)
    ~(y  : int)
...
let set_node_shape
  ...
  in
  (cy##style##selector (Js.string (Printf.sprintf "node[id=\"%s\"]" id)))
  ##style (Js.string "shape") shape |> ignore;
  if update_style then
    style_update cy
  else
    ()

let enable_node_border
    ?(update_style : bool = true)
    (cy : t Js.t)
    ~(id : string)
  : unit =
  ...
  (cy##style##selector (Js.string (Printf.sprintf "node[id=\"%s\"]" id)))
   ##style (Js.string "border-width") width |> ignore;
  if update_style then
    style_update cy
  else
    ()

let disable_node_border
    ?(update_style : bool = true)
    (cy : t Js.t)
    ~(id : string)
  : unit =
...
let start_batch (cy : t Js.t) : unit =
  cy##startBatch

let end_batch (cy : t Js.t) : unit =
  cy##endBatch
