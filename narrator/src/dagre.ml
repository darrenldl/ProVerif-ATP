...
let set_node ?(height : int = 100) (graph : t Js.t) ~(id : string)
    ~(label : string) : unit =
...
let remove_node (graph : t Js.t) ~(id : string) : unit =
  graph##removeNode (Js.string id)

let set_edge (graph : t Js.t) ~(source : string) ~(target : string) : unit =
  graph##setEdge (Js.string source) (Js.string target)

let remove_edge (graph : t Js.t) ~(source : string) ~(target : string) : unit =
  graph##removeEdge (Js.string source) (Js.string target)

let get_node_xy (graph : t Js.t) ~(id : string) : int * int =
  let node = graph##node (Js.string id) in
  (node##.x, node##.y)

let layout (graph : t Js.t) : unit =
